...
@Test func scanFailClosed_emptyNonUTF8OrJSONLessThrows() {
    ...
    #expect(throws: ScanStoreError.unreadable(sourcePath: source)) {
        _ = try ScanJSONLEngine.extractFailClosed(
            ...
            profile: ScanJSONLProfile(sessionKeys: ["session_id"], timestampKeys: ["timestamp"], allowEpochTimestamp: true, commands: { _ in [] })
        )
    }
    #expect(throws: ScanStoreError.unreadable(sourcePath: source)) {
        _ = try ScanJSONLEngine.extractFailClosed(
...
@Test func scanFailClosed_assemblesSessionTimestampCwdPerLine() throws {
    ...
    let events = try ScanJSONLEngine.extractFailClosed(
        ...
        profile: ScanJSONLProfile(sessionKeys: ["session_id", "sessionId"], timestampKeys: ["timestamp", "ts"], allowEpochTimestamp: true, commands: { _ in ["echo hi"] })
    )
...
@Test func scanFailClosed_fallsBackToFileSession() throws {
    let events = try ScanJSONLEngine.extractFailClosed(
        ...
        profile: ScanJSONLProfile(sessionKeys: ["conversation_id", "session_id", "sessionId"], timestampKeys: ["timestamp", "ts"], allowEpochTimestamp: false, commands: { _ in ["ls"] })
    )
