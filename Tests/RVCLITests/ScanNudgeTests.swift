...
private func wirePiAdapter(homeURL: URL) throws {
    ...
    let body = try SetupHostKind.pi.adapterResource().rendered(rvPath: executable.path)
...
private func runPiScan(home: ScanHome) throws -> ScanReport {
    try ScanRun.run(
        ScanRun.Request(
            rootPath: nil,
            home: home,
...
@Test func scanFailOnFindings_withoutFlag_exitsZeroWithFindings() throws {
    try withTempScanHome { home, homeURL in
        ...
        let report = try runPiScan(home: home)
        #expect(report.findings.isEmpty == false)
...
@Test func scanSetupNudge_wiredHost_doesNotRecommend() throws {
    try withTempScanHome { home, homeURL in
        ...
        let report = try runPiScan(home: home)
        #expect(report.setupNudgeRecommended == false)
...
@Test func scanSetupNudge_claudeOnly_noSetupHost_doesNotRecommend() throws {
    try withTempScanHome { home, homeURL in
