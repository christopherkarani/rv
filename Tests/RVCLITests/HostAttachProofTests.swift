...
/// The oracle is `tools/host-attach-proof.sh`; this suite does not run it
...
struct HostAttachProofTests {
    @Test func hostAttachProof_scriptAndCIWireTheThreeHostAttach() throws {
        ...
        let script = root.appendingPathComponent("tools/host-attach-proof.sh")
        ...
        #expect(pr.contains("tools/host-attach-proof.sh"))
        ...
        let attach = pr.range(of: "tools/host-attach-proof.sh")
        #expect(linuxJob != nil && darwinJob != nil && attach != nil)

        let prAttachCount = pr.components(separatedBy: "tools/host-attach-proof.sh").count - 1
        ...
        #expect(release.contains("tools/host-attach-proof.sh"))
    }
}
