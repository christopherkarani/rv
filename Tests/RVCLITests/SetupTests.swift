...
@Test func setup_grokOnly_writesOwnedPayloadWithBakedPath() throws {
    try withTempHome { home, layout, launchctl in
        ...
        #expect(body.contains("\"matcher\"") == false)
...
@Test func setup_cursorOnly_writesAdapterAndMergesHooksJSON() throws {
    try withTempHome { home, layout, launchctl in
        ...
        #expect(preToolUse[0]["command"] as? String == "python3 \(layout.cursorHook)")
        ...
        #expect(afterHooks["preToolUse"] == nil)
    }
}
...
@Test func setup_claudeOnly_mergesFingerprintAndPreservesForeignEntry() throws {
    try withTempHome { home, layout, launchctl in
        ...
        #expect(entries[3]["matcher"] as? String == "Edit")
