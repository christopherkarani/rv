...
struct ServiceStatusTests {
    @Test func robotAndPlainFieldsForDown() async {
        let report = await ServiceClient(transport: nil).status()
    ...
    @Test func runningReportsInactiveFallback() async {
        let transport = ScriptedTransport(
            ack: HelloAckView(protocolName: "rv.ipc.v1", serviceSemver: "1.0.0", ok: true)
        )
        let report = await ServiceClient(transport: transport).status()
