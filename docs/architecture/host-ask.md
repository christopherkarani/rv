...
RV: `rv-guard-openclaw.js.tmpl` + `OpenClawHostCodec` (`HostAskCodec`). Matcher `["exec"]`. Handler `timeoutMs` is the 120s wait plus slack (default `before_tool_call` budget is 15s fail-closed and does not cancel the handler). Live deny: `{ block: true, blockReason }` (operator `{decision,reason}` JSON, exit 1). Ask: `decision:ask` → plugin-owned wait (`plugin.approval.request` + `waitDecision`, tests `RV_ASK_CONFIRM`) → `hostAsk=spend` → return only if spend allow. Abort or confirm-no skips spend. Never return `requireApproval` (host Allow runs exec). `allow-always` is not this grant. Tests forbid `requireApproval`.
