...
| **RVHooks** | **Pi / Grok / OpenCode / Claude** Host adapters: shell codecs, Claude rich deny builder, Hook mapper/voice, embedded adapter resources | evaluation, setup mutations |
...
| **RVCLI** | ArgumentParser, output mode, thin XPC client, typed service diagnostics, service health facts, GatedEvaluate for TTY test/explain and hook XPC miss, Host adapter installation state + setup mutations (incl. Claude `settings.json` merge) | regex, pack parse |
...
| `RVHooks` | `RVDomain` | Complete Pi/Grok/OpenCode/Claude Host adapter behavior (`ClaudeHostCodec` + rich deny); no setup mutations. |
...
| `RVCLI` | `RVDomain`, `RVEngine`, `RVPolicy`, `RVHooks`, `RVIPC`, `RVPresentation`, `RVTheme`, `RVTUI`, `RVService`, `RVHistory`, `RVAnalytics` | Thin client; typed service diagnostics; service health facts shared by doctor and status; GatedEvaluate for TTY test/explain and hook XPC miss; read-only Host adapter installation state shared by setup and doctor; Claude settings merge/uninstall in `SetupRun`. No regex, no pack parse. |
