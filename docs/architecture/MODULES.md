...
| **RVCLI** | ArgumentParser, output mode, thin XPC client, constructs EvaluateSession on miss | regex, pack parse |
...
| `RVCLI` | `RVDomain`, `RVEngine`, `RVPolicy`, `RVHooks`, `RVIPC`, `RVPresentation`, `RVTheme`, `RVTUI`, `RVService`, `RVHistory` | Thin client; constructs EvaluateSession on miss. No regex, no pack parse. |
