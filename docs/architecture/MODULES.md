...
**Protected host paths (OPE-160).** `SecretPathCatalog.dayOne` is the single matcher (categories are inspectable). A catalog hit is `FilesystemScope.protectedPath` and `builtin.action:protected-path-mutation` — overlay / allowlist / always-allow cannot weaken it. `~`, `$HOME`, and `${HOME}` expand like each other; symlink and `..` use the resolved target. Ordinary in-repo files stay on the repository-boundary path. Pack deny / indeterminate stays a floor.
