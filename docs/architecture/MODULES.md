...
| **RVPolicy** | config merge, allowlist, allow-once, `HostGrantWriter`, durable `PendingApprovalStore`, Apple Foundation Models `ActionReviewer` adapter (shadow), `ShadowReviewRunner`, `TypedRule` / `policy.toml` store (load/save/merge), `FoundationModelsEnglishCompiler` | rendering |
