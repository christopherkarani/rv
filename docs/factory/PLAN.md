...
25. **One CLI surface.** The only user command is `rv`. `rvd` is the service, not a CLI. The Swift operator’s on-disk sibling name (`rv-cli` today) may appear in install/C/doctor internals. It must not appear in `README.md`, help catalogs, or install hero copy as a command to type. Vocabulary: `CONTEXT.md` (CLI surface / hook client / operator).
