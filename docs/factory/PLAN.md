...
- `rv uninstall`: removes only rv-owned files. TTY pretty paces host removal (`Removing Hooks` → hollow circles → `✓ Hooks removed` → `Uninstall complete`). Occupied owned names stay hollow with `left occupied`. Already clean: `Already clean`. Robot: `Uninstall complete.` / `Already clean.` One line, no sleeps.
...
24. **Uninstall show.** TTY pretty paces owned-hook removal (`Removing Hooks` → empty circles → `Uninstall complete`). Occupied names are left alone (`left occupied`). Robot is one line. Never deletes foreign hooks.
