...
| Done | session scan T1–T10 | Stack `arch/3c4b611b`. `rv scan` / `rv scan sessions` session forensics. Spec: [`spec/spec-architecture-session-scan.md`](../../spec/spec-architecture-session-scan.md). |
...
| Done | session scan T1; then T2; then T3 ∥ T4 ∥ T5; then T6; then T7; then T8; then T9; then T10 | Stack `arch/3c4b611b`. Exclusive files in `spec/spec-architecture-session-scan.md` § 5b |
