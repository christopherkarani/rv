...
| `redirect-truncate-dynamic-path` final backtick | 0.11.0 treats a last-character `` ` `` as expansion | `$`/`\` anywhere; backtick at start of the target token **or interior** (a non-whitespace token character after the opener in the same token). Keywords `` >` `` / `` > ` `` keep the pack a candidate (same shape as `>$` / `> $`). Not a final markdown closer, and not a closer from a later span. Allow: `near.quoted-heredoc-angle-placeholder`, `near.quoted-heredoc-angle-placeholder-two-span`. |
